...
_base_ = [
    '../../_base_/datasets/Cityscapes_2048_1024_3.py' #,  #数据集配置  by FRC
]
...
model = dict(
    ...
    backbone=dict(
        ...
        init_cfg=dict(
            type='Pretrained',
            checkpoint='../data/pretrained_models/segformer/mit_bs0.pth')),
    decode_head=dict(
        type='SegformerHead',
    ...
    # 解码头(decode_head)里的损失函数的配置项。   FocalLoss、LovaszLoss、CrossEntropyLoss、DiceLoss   use_sigmoid
...
optimizer = dict(type='AdamW', lr=6e-3, betas=(0.9, 0.999), weight_decay=0.01,
