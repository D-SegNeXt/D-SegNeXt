_base_ = [
    ...
    '../../_base_/datasets/CMMPNETVOC_1024_4.py' #,  #数据集配置  by FRC
...
model = dict(
    ...
    backbone=dict(
        type='SegFormerB0',      #主干网的类别   by FRC
        in_chans=4,
        style='pytorch'),
    decode_head=dict(
        type='SSegformerHead',   # 解码头(decode head)的类别   by FRC  SSegformerHead   SegUnetFormerHeadB0
        ...
        loss_decode=dict(type='CrossEntropyLoss', use_sigmoid=False, loss_weight=1.0, class_weight= [0.8424, 1.1575])),# 解码头(decode_head)里的损失函数的配置项。   FocalLoss、LovaszLoss、CrossEntropyLoss、DiceLoss
