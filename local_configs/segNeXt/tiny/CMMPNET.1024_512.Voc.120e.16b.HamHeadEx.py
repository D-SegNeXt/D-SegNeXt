...
model = dict(
    ...
    backbone=dict(
         init_cfg=dict(type='Pretrained', checkpoint='../data/pretrained_models/segNeXtTinyAttentionBackbone.pth')), #'../data/pretrained_models/mscan_t.pth')
    decode_head=dict(
        type='LightHamHeadEx',
...
optimizer = dict(type='AdamW', lr=6e-4, betas=(0.9, 0.999), weight_decay=0.01,
                 paramwise_cfg=dict(custom_keys={'pos_block': dict(decay_mult=0.),
                                                 'norm': dict(decay_mult=0.),
                                                 'head': dict(lr_mult=5.) # head 的 LR 是 backbone 的 10 倍
                                                 }))
