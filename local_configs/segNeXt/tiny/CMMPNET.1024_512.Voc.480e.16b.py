...
model = dict(
    ...
    backbone=dict(
         init_cfg=dict(type='Pretrained', checkpoint='../data/pretrained_models/segNeXtTinyAttentionBackbone.pth')), #'../data/pretrained_models/mscan_t.pth') segNeXtTinyAttention
    decode_head=dict(
        type='LightHamHead',
...
#RunTime
log_config = dict(
    interval=500,  #每 interval 个迭代Iteration输出一次log
    hooks=[
...
runner = dict(type='EpochBasedRunner', max_epochs=300) #按epoch的方式进行迭代runner = dict(type='EpochBasedRunner', max_epochs=30) #按epoch的方式进行迭代
