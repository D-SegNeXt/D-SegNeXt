...
model = dict(
    ...
    decode_head=dict(
        ...
        norm_cfg=norm_cfg,
        align_corners=False,
        loss_decode=dict(type='CrossEntropyLoss', use_sigmoid=False, loss_weight=0.4, class_weight=[0.8424, 1.1575])),
    auxiliary_head=dict(
        type='FCNHead',
        in_channels=160,
        in_index=2,
        ...
        norm_cfg=norm_cfg,
        align_corners=False,
        loss_decode=dict(type='CrossEntropyLoss', use_sigmoid=False, loss_weight=1, class_weight=[0.8424, 1.1575])),
...
optimizer = dict(type='AdamW', lr=6e-4, betas=(0.9, 0.999), weight_decay=0.01,
                 paramwise_cfg=dict(custom_keys={'pos_block': dict(decay_mult=0.),
                                                 'norm': dict(decay_mult=0.),
                                                 'head': dict(lr_mult=1.) # head 的 LR 是 backbone 的 10 倍
                                                 }))
