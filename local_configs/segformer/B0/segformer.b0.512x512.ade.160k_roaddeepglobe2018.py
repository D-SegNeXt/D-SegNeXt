_base_ = [
    ...
    '../../_base_/datasets/road_deepglobe2018.py',  #数据集配置  by FRC
...
model = dict(
    ...
    decode_head=dict(
        ...
        loss_decode=dict(type='CrossEntropyLoss', use_sigmoid=False, loss_weight=10.0)),# 解码头(decode_head)里的损失函数的配置项。
...
data = dict(samples_per_gpu=2)   #samples_per_gpu为batch的size，即是多少个样本为一个batch
