...
class MyNormalize(object):
    ...
    def __call__(self, results):
        ...
        # COLOR_BGRA2RGBA
        import cv2
        _img = results['img'].copy().astype(np.float32)
        assert _img.dtype != np.uint8
        _mean = np.float64(self.mean.reshape(1, -1))
        _stdinv = 1 / np.float64(self.std.reshape(1, -1))
        cv2.cvtColor(_img, cv2.COLOR_BGRA2RGBA, _img)  # inplace
        cv2.subtract(_img, _mean, _img)  # inplace
        cv2.multiply(_img, _stdinv, _img)  # inplace
...
class PhotoMetricDistortion(object):
    ...
    def __call__(self, results):
        ...
        if mode == 0:
            img = self.contrast(img)
        img = np.concatenate([img, alphaimg], 2)
        results['img'] = img
        return results
