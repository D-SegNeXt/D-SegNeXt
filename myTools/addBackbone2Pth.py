...
if __name__ == '__main__':
    ...
    _parser.add_argument('--inPthPath', default='../data/pretrained_models/segNeXt/mscan_t.pth', help='in pth path')
    _parser.add_argument('--outPthPath', default='../data/pretrained_models/segNeXt/mscan_t_addBackbone.pth', help='out pth path')
    ...
    if 'state_dict' in segNeXtTinyAttention:
        ...
        for key, value in _state_dict.items():
            if key == 'head.weight':
                _outstate_dict['head.fc.weight'] = value
            elif  key == 'head.bias':
                _outstate_dict['head.fc.bias'] = value
            else:
                _key = 'backbone.' + key
                _outstate_dict[_key] = value
